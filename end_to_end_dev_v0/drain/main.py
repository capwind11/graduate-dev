...
def optimize_by_seq_dist():
    ...
    opt = Optimizer()
    opt.modify(method = 'seq_dist', tree = drain.prefixTree, logparser=drain, st = 0.7)
    logClusters = drain.logClusters
    printClusters(logClusters)
    root = drain.copy()
    createPlot(root)
...
def optimize_by_merge_sub_tree():
    ...
    print('优化前')
    printClusters(logClusters)
    opt.modify(method='merge_sub_tree', tree=drain.prefixTree, logparser= drain)
    logClusters = drain.logClusters
    print('合并子树优化')
    printClusters(logClusters)
    # createPlot(drain)
    opt.modify(method = 'seq_dist', tree = drain.prefixTree, logparser=drain, st = 0.8)
    logClusters = drain.logClusters
    print('合并聚类优化')
    printClusters(logClusters)
    drain.save()
...
def optimize_by_tfidf():
    ...
    printClusters(logClusters)
    opt = Optimizer()
    opt.modify(method = 'tfidf', tree = drain.prefixTree, logparser=drain, st = 0.6)
    logClusters = drain.logClusters
    print( )
    printClusters(logClusters)
    root = drain.copy()
